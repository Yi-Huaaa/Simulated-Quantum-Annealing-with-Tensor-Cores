...
void construct_spin(float *x, float *x_buf, float *y, float *y_buf){
    for (int n = 0; n < N; n++){  
    ...
    cudaErrCheck (cudaMemcpy(x_buf, x, N*16*sizeof(float), cudaMemcpyHostToDevice));
...
void UpdateTwice_CPU (float *x, float *y, float amplitude, float *x_buf, float *y_buf){
...
int print_energy(float *x, float *x_buf, float *couplings, double curr, int t)
{
	//GPU version
