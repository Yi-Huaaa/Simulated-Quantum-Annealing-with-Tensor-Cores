...
// SQA parameters
#define N 32768 // 1024 2048 4096 8192 16384 32768
#define M 64
...
void update_delta_H(cublasHandle_t cublasHandle, half *couplings_fp16, half *matrix_B_fp16, float *delta_H_fp32, int which_spin){
    ...
    unsigned long long int blk_num = which_spin / M;
    //int loop_iter = (N/32768 == 0) ? 1 : N/32768; 

    //for (int i = 0; i < loop_iter; i++) {
        unsigned long long int coup_idx = 0;
        cublasErrCheck(cublasGemmEx(cublasHandle, CUBLAS_OP_N, CUBLAS_OP_N, N, M, M, &alpha, couplings_fp16, CUDA_R_16F, N, matrix_B_fp16, CUDA_R_16F, M, &beta, delta_H_fp32, CUDA_R_32F, N, CUBLAS_COMPUTE_32F, CUBLAS_GEMM_DEFAULT_TENSOR_OP));
    //}
}
...
__global__ void judge_flipping_com (half *couplings_fp16, float *delta_H_fp32, float *spin_fp32, half *matrix_B_fp16, float *log_rand_val_fp32, int J_perp, float beta, int start_spin){
    ...
    // even: 0~M/2-1; odd: M/2~M-1
    //#pragma unroll
    //for (int n = 0; n < M; n++) {
        // int nn = start_spin + ((first_rd_idx*(M/2) + n)&(M-1));
        int nn = start_spin;
        idx = IDX2C(nn,m,N);
        mb_idx = IDX2C(nn&(M-1),m,M);      
        delta = delta_H_fp32[IDX2C(start_spin+threadIdx.x, m, N)];
...
int main(int argc, char* argv[]) {
    ...
    half *matrix_B_fp16;
    cudaErrCheck(cudaMalloc((void**)&matrix_B_fp16, M*M*sizeof(float)));
    ...
    for (int t = 0; t < TIMES; t++) {
        ...
        for (int p = 0; p < STEP; p++) {
            ...
            for (int n = 0; n < N; n ++) {
                judge_flipping_com <<< 16, 1, 0 >>> (couplings_fp16, delta_H_fp32, 
                    spin_fp32, matrix_B_fp16, log_rand_val_fp32, J_perp, 2*M*beta, n);
                update_delta_H(cublasHandle, couplings_fp16, matrix_B_fp16, delta_H_fp32, 0);              
            }
            beta += increase;
