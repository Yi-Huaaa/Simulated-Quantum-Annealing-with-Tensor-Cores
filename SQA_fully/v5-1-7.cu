/*
和 516差別：
(1) matrixB 32x16->16x16
(2) thread <<<1,16>>> -> <<<16,1>>>
(3) fix solution quality

G1 800 spins
time from 0.09s  -> 0.09s (100step)
          0.9ms -> 9.0ms (per step)

G22 2000 spins
time from 0.19s  -> 0.21s (100step)
          1.9ms -> 2.1ms (per step)

G48 3000 spins
time from 0.60s  -> 0.44s (100step)
          6.0ms -> 4.4ms (per step)

G65 8000 spins
time from 1.15s  -> 0.91s (100step)
          11.5ms -> 9.1ms (per step)

G77 14000 spins
time from 2.42s  -> 1.95s (100step)
          24.2ms -> 19.5ms (per step)

G81 20000 spins
time from 25.05s  -> 4.44s (100step)
          250ms -> 44.4ms (per step)
*/
...
void construct_lograndval(float *log_rand_val, float *log_rand_val_fp32, float beta){
    ...
    for(int i = 0; i < M; i++){
        for(int j = 0; j < M; j++){
            log_rand_val[IDX2C(i,j,M)] = (-log(((float)rand()/(float)(RAND_MAX)) * 1.0)) / beta;
        }
    }
    cudaErrCheck (cudaMemcpy(log_rand_val_fp32, log_rand_val, M*M*sizeof(float), cudaMemcpyHostToDevice));
}
...
__device__ void flip_com (float *couplings_fp32,float *delta_H_fp32, float *spin_fp32, float *matrix_B_fp32, float *log_rand_val_fp32, int J_perp, float beta, int start_spin, int m){
    ...
    for(int n = (start_spin + first_rd_idx * 8); n < (start_spin + 8 + first_rd_idx * 8); n++){
        idx = IDX2C(n,m,N);
        mb_idx = IDX2C(n%M,m,M);            
        delta = delta_H_fp32[idx];
        
        for(int i = start_spin + first_rd_idx*8; i < n; i++){
            delta += couplings_fp32[IDX2C(i,n,N)]*matrix_B_fp32[IDX2C(i%M, m, M)];
        }
    ...
    for(int n = (start_spin + second_rd_idx * 8); n < (start_spin + 8 + second_rd_idx * 8); n++){
        idx = IDX2C(n,m,N);
        mb_idx = IDX2C(n%M,m,M);            
        delta = delta_H_fp32[idx];
        
        //更新delta: 上面後16個spin做的
        for(int i = (start_spin + first_rd_idx * 8); i < (start_spin + 8 + first_rd_idx * 8); i++){
            delta += couplings_fp32[IDX2C(i,n,N)]*matrix_B_fp32[IDX2C(i%M, m, M)];
        }
        //更新delta: 本輪做的
        for(int i = (start_spin + second_rd_idx * 8); i < n; i++){
            delta += couplings_fp32[IDX2C(i,n,N)]*matrix_B_fp32[IDX2C(i%M, m, M)];
        }
...
__global__ void judge_flipping_com (float *couplings_fp32, float *delta_H_fp32, float *spin_fp32, float *matrix_B_fp32, float *log_rand_val_fp32, int J_perp, float beta, int start_spin){
    ...
    flip_com(couplings_fp32, delta_H_fp32, spin_fp32, matrix_B_fp32, log_rand_val_fp32, J_perp, beta, start_spin, idx);
}
...
int main(int argc, char* argv[]) {
    ...
    float *log_rand_val;
    log_rand_val = (float*)malloc(M*M*sizeof(float));
    memset(log_rand_val, 0, M*M*sizeof(float));

    float *log_rand_val_fp32;
    cudaErrCheck(cudaMalloc((void**)&log_rand_val_fp32, M*M*sizeof(float)));
    cudaErrCheck(cudaMemcpy(log_rand_val_fp32, log_rand_val, M*M*sizeof(float), cudaMemcpyHostToDevice));
    ...
    for (int t = 0; t < TIMES; t++) {
        ...
        for (int p = 0; p < STEP; p++) {
            ...
            for (int n = 0; n < N; n+=16) {
                ...
                judge_flipping_com <<< 16, 1, 0 >>> (couplings_fp32, delta_H_fp32, spin_fp32, matrix_B_fp32, log_rand_val_fp32, J_perp, beta, n);
