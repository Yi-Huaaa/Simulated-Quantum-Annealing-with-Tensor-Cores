...
// SQA parametersl
#define N 1024 // 1024 2048 4096 8192 16384 32768, 6
...
__global__ void construct_delta_H(nv_bfloat16 *couplings_fp16, float *spin_fp32, float *delta_H_fp32){
...
void update_delta_H(cublasHandle_t cublasHandle, nv_bfloat16 *couplings_fp16, nv_bfloat16 *matrix_B_fp16, float *delta_H_fp32, int which_spin){
    ...
    for (int i = 0; i < loop_iter; i++) {
        ...
        cublasErrCheck(cublasGemmEx(cublasHandle, CUBLAS_OP_N, CUBLAS_OP_N,
                                    ...
                                    &alpha,
                                    couplings_fp16 + coup_idx, CUDA_R_16BF, matrix_m,
                                    matrix_B_fp16, CUDA_R_16BF, MATRIX_K,
                                    &beta,
...
float calculate_maxcut (float *spin, float *spin_fp32, nv_bfloat16 *couplings, int total_spins, int total_couplings){
...
__global__ void judge_flipping_com (nv_bfloat16 *couplings_fp16, float *delta_H_fp32, float *spin_fp32, nv_bfloat16 *matrix_B_fp16, float *log_rand_val_fp32, int J_perp, float beta, int start_spin){
    int m = blockIdx.x;
...
int main(int argc, char* argv[]) {
    ...
    nv_bfloat16 *couplings; // cpu    
    couplings = (nv_bfloat16*)malloc(N * N * sizeof(nv_bfloat16));
    memset(couplings, 0, N*N*sizeof(nv_bfloat16));
    
    nv_bfloat16 *couplings_fp16; 
    cudaErrCheck(cudaMalloc((void**)&couplings_fp16, N*N*sizeof(nv_bfloat16)));
    ...
    cudaErrCheck ( cudaMemcpy(couplings_fp16, couplings, N*N*sizeof(nv_bfloat16), cudaMemcpyHostToDevice) );
    ...
    nv_bfloat16 *matrix_B_fp16;
    cudaErrCheck(cudaMalloc((void**)&matrix_B_fp16, M*M_2*sizeof(nv_bfloat16)));
