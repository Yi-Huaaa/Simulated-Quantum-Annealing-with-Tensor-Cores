...
parallel: odd, even do @ z same time
		  1,16,0: 最快，但不知道為啥只能用1,8,0才可以用ＱＱ
目前結果：
相當快，能量也正確
*/
#include <stdio.h>
...
/*
我開了8個thread去寫，總共有16個東西要變換，那
*/
__global__ void judge_flipping_even (float *couplings_fp32,float *delta_H_fp32, float *spin_fp32, float *matrix_B_fp32, float *rand_val_fp32, int J_perp, float beta, int start_spin){
    int idx = 0, mb_idx = 0, upper = 0, lower = 0;
    float delta = 0.;
    int m = (blockIdx.x * blockDim.x + threadIdx.x)*2;
    int sixteen = start_spin+16;
    int changed[16] = {0};
    int changed_id = 0, in_loop_changed_id;
...
__global__ void judge_flipping_odd (float *couplings_fp32,float *delta_H_fp32, float *spin_fp32, float *matrix_B_fp32, float *rand_val_fp32, int J_perp, float beta, int start_spin){
    int idx = 0, mb_idx = 0, upper = 0, lower = 0;
    float delta = 0.;
    int m = (blockIdx.x * blockDim.x + threadIdx.x)*2 + 1;
    int sixteen = start_spin+16;
    int changed[16] = {0};
    int changed_id = 0, in_loop_changed_id;
    ...
    for(int n = start_spin; n < sixteen; n++){
        ...
        //更新delta
        for(int pre_n = start_spin; pre_n < n; pre_n++){ //j: 這一輪中前面的所有spin
            delta += 2*couplings_fp32[IDX2C( pre_n, n, N)]*spin_fp32[IDX2C( pre_n, m, N)];
        }
    ...
    idx = IDX2C(start_spin,m,N);
    mb_idx = IDX2C(start_spin%M,m,M);            
    delta = delta_H_fp32[idx];
    upper = (m == 0 ? M-1 : m-1);
    lower = (m == M-1 ? 0 : m+1);
    delta = 2*M*spin_fp32[idx]*(delta - M*J_perp*(spin_fp32[IDX2C(start_spin,upper,N)] + spin_fp32[IDX2C(start_spin,lower,N)]));
    if ( (-log(rand_val_fp32[idx]) / beta) > delta ) {
        spin_fp32[idx] = -spin_fp32[idx];
        matrix_B_fp32[mb_idx] = 2*spin_fp32[idx];
    } */
    ...
    for (int t = 0; t < TIMES; t++) {
        ...
        for (int p = 0; p < STEP; p++) {
            ...
            for (int n = 0; n < N; n+=16) {
                ...
                judge_flipping_odd <<< 1,8,0 >>> (couplings_fp32, delta_H_fp32, spin_fp32, matrix_B_fp32, rand_val_fp32, J_perp, beta, n);                
